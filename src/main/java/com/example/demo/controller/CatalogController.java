...
public class CatalogController {
    ...
    public ResponseEntity<?> addCrop(@RequestBody CropRequest req, Authentication auth) {
        ...
        Crop c = catalogService.addCrop(Crop.builder().name(req.getName()).suitablePHMin(req.getSuitablePHMin())
    ...
    public ResponseEntity<?> addFertilizer(@RequestBody FertilizerRequest req, Authentication auth) {
        if(!auth.getAuthorities().toString().contains("ROLE_ADMIN")) return ResponseEntity.status(403).build();
        Fertilizer f = catalogService.addFertilizer(Fertilizer.builder().name(req.getName()).npkRatio(req.getNpkRatio())
